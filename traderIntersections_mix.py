...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        self.logger.info("received order book for instrument %d with sequence number %d", instrument,
                         sequence_number)
        if sequence_number < 2: #Aspettiamo un pochino
            return
        ...
        if instrument == Instrument.ETF:
            ...
            diff = meanETF - meanFUT
            #print(meanETF - meanFUT)
            if diff >= 700:
                if self.ETF.getMaxBid() > 0 and self.checkLots(Instrument.ETF, - LOT_SIZE - 30):
                    ask_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(ask_id, Side.SELL, self.ETF.getMaxBid(), LOT_SIZE + 30, Lifespan.FILL_AND_KILL)
                    self.asks[ask_id] = [self.ETF.getMaxBid(), LOT_SIZE + 30, time.time()]
            if diff == 600:
                if self.ETF.getMaxBid() > 0 and self.checkLots(Instrument.ETF, - LOT_SIZE - 25):
                    ask_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(ask_id, Side.SELL, self.ETF.getMaxBid(), LOT_SIZE + 25, Lifespan.FILL_AND_KILL)
                    self.asks[ask_id] = [self.ETF.getMaxBid(), LOT_SIZE + 25, time.time()]
            if diff == 500:
            ...
            if diff == 400:
                if self.ETF.getMaxBid() > 0 and self.checkLots(Instrument.ETF, - LOT_SIZE - 10):
                    ask_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(ask_id, Side.SELL, self.ETF.getMaxBid(), LOT_SIZE + 10, Lifespan.FILL_AND_KILL)
                    self.asks[ask_id] = [self.ETF.getMaxBid(), LOT_SIZE + 10, time.time()]
            if diff == -400:
                if self.ETF.getMinAsk() > 0 and self.checkLots(Instrument.ETF, LOT_SIZE + 10):
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, self.ETF.getMinAsk(), LOT_SIZE + 10, Lifespan.FILL_AND_KILL)
                    self.bids[bid_id] = [self.ETF.getMinAsk(), LOT_SIZE + 10, time.time()]
            if diff == -500:
                if self.ETF.getMinAsk() > 0 and self.checkLots(Instrument.ETF, LOT_SIZE + 20):
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, self.ETF.getMinAsk(), LOT_SIZE + 20, Lifespan.FILL_AND_KILL)
                    self.bids[bid_id] = [self.ETF.getMinAsk(), LOT_SIZE + 20, time.time()]
            if diff == -600:
                if self.ETF.getMinAsk() > 0 and self.checkLots(Instrument.ETF, LOT_SIZE + 25):
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, self.ETF.getMinAsk(), LOT_SIZE + 25, Lifespan.FILL_AND_KILL)
                    self.bids[bid_id] = [self.ETF.getMinAsk(), LOT_SIZE + 25, time.time()]
            if diff <= -700:
                if self.ETF.getMinAsk() > 0 and self.checkLots(Instrument.ETF, LOT_SIZE + 30):
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, self.ETF.getMinAsk(), LOT_SIZE + 30, Lifespan.FILL_AND_KILL)
                    self.bids[bid_id] = [self.ETF.getMinAsk(), LOT_SIZE + 30, time.time()]
