...
from MarketTracer01 import MarketTracer
...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.fut = 0
        self.soldi = 2000000 #aviable money
    ...
    def checkUnhedged(self):
        ...
        """
        if self.etfs +self.fut < -10:
            request_price = self.FUT.getMinAsk()
            volume = abs(self.etfs+self.fut+10)
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            self.checkUnhedged()
            if minAsk > maxBid:
                if minAsk-100 > 0 and self.checkLots(Instrument.ETF, -LOT_SIZE) and self.checkCrossing(self.asks, minAsk - 100):
                    print("SELL")
                    ask_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(ask_id,Side.SELL,minAsk - 100, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                    self.asks[ask_id]=[minAsk - 100, LOT_SIZE]
                if maxBid +100 > 0 and self.checkLots(Instrument.ETF, LOT_SIZE) and self.checkCrossing(self.bids, maxBid + 100):
                    print("BUY")
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, maxBid + 100, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                    self.bids[bid_id]=[maxBid+100,LOT_SIZE]
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.asks:
            self.etfs -= volume
            self.soldi += volume*price
            request_price = self.FUT.getMaxBid() + 100
            if request_price and self.checkLots(Instrument.FUTURE, volume):
                print("HBID")
                bid_id = next(self.order_ids)
                self.checkOperations()
        ...
        if client_order_id in self.bids:
            self.etfs += volume
            self.soldi -= volume*price
            request_price = self.FUT.getMinAsk() - 100
            if request_price and self.checkLots(Instrument.FUTURE, -volume):
                print("HASK")
                ask_id = next(self.order_ids)
                self.checkOperations()
