    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if minAsk > maxBid:
                ...
                if askPrice > 0 and self.checkLots(Instrument.ETF, -LOT_SIZE) and self.checkCrossing(self.asks,
                                                                                                         askPrice):
                    print("SELL")
                    ask_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(ask_id, Side.SELL, askPrice, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                    self.asks[ask_id] = [askPrice, LOT_SIZE]
                if bidPrice > 0 and self.checkLots(Instrument.ETF, LOT_SIZE) and self.checkCrossing(self.bids,
                                                                                                        bidPrice):
                    print("BUY")
                    bid_id = next(self.order_ids)
                    self.checkOperations()
                    self.send_insert_order(bid_id, Side.BUY, bidPrice, LOT_SIZE, Lifespan.GOOD_FOR_DAY)
                    self.bids[bid_id] = [bidPrice, LOT_SIZE]
